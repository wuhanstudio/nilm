...
static int push_to_transition_data(EdgeDetector *detector, float value) {
...
EdgeDetectorOutput edge_detector_update(
    ...
    state_change = fabsf(current_measurement - detector->previous_measurement);
    ...
    if (detector->instantaneous_change_queue_len == detector->min_n_samples &&
        ...
        float last_transition = detector->estimated_steady_power - detector->last_steady_power;

        if (fabsf(last_transition) > detector->noise_level) {
    ...
    detector->estimated_steady_power =
        ((float)detector->N * detector->estimated_steady_power + current_measurement) /
        (float)(detector->N + 1);
