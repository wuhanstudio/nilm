...
float X_norm[REDD_FEATURES];
float* redd_normalize(const float* X) {
    for (int i = 0; i < REDD_FEATURES; i++) {
        X_norm[i] = (X[i] - REDD_X_MEAN) / REDD_X_STD;
...
static int redd_booleanize_n_bit(float x, int num_bits, uint8_t *out_bits) {
    if (x < 0.0f || x > 1.0f)
        return -1;
...
uint8_t* redd_booleanize_features(
    ...
    uint8_t* X_bool = malloc(REDD_FEATURES * num_bits * sizeof(uint8_t));
    if(!X_bool) {
        return NULL;
    }
    ...
    int offset = 0;
    for (int i = 0; i < REDD_FEATURES; i++) {
        redd_booleanize_n_bit(X[i], num_bits, &X_bool[offset]);
        offset += num_bits;
    }
    return X_bool;
}
